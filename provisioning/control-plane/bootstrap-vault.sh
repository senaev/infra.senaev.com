...
for i in $(seq 1 "$MAX_ATTEMPTS"); do
  ...
  if [[ -n "$STATUS_JSON" ]]; then
    echo "✅ [bootstrap-vault] Vault is responding."
    break
  fi
  echo "   ⏳ Pending: waiting for Vault to start ($i/$MAX_ATTEMPTS)"
  if [[ "$i" -eq "$MAX_ATTEMPTS" ]]; then
    echo "❌ [bootstrap-vault] Vault did not respond in time. Check pod logs/status."
    exit 1
  fi
  sleep 1
done
...
echo "👉 [bootstrap-vault] Refreshing vault status"
STATUS_JSON="$(status_json)"
SEALED="$(echo "$STATUS_JSON" | jq -r '.sealed')"

echo "✅ [bootstrap-vault] SEALED=[$SEALED]"

if [[ "$SEALED" == "true" ]]; then
  if [[ ! -f "$INIT_FILE" ]]; then
    echo "❌ [bootstrap-vault] Vault is sealed but $INIT_FILE not found. Can't unseal automatically."
    exit 1
  fi

  echo "👉 [bootstrap-vault] Vault is sealed. Unsealing"

  KEY="$(jq -r ".unseal_keys_b64[0]" "$INIT_FILE")"
  vault_exec operator unseal "$KEY" >/dev/null
  echo "✅ [bootstrap-vault] Unseal done."
else
  echo "✅ [bootstrap-vault] Vault is already unsealed."
fi

echo "👉 [bootstrap-vault] Checking final vault status"
FINAL_STATUS="$(vault_exec status)"
echo "✅ [bootstrap-vault] Final vault status:"
echo "--------------------------------"
echo "$FINAL_STATUS"
echo "--------------------------------"

echo ""
echo "⬇️ [bootstrap-vault] Setup Vault Secrets Management for External Secrets Operator and Kubernetes auth"
echo ""

echo "👉 Getting root token from INIT_FILE=[$INIT_FILE]"
if [[ ! -f "$INIT_FILE" ]]; then
  echo "❌ [bootstrap-vault] INIT_FILE not found"
  exit 1
fi
ROOT_TOKEN="$(jq -r '.root_token' "$INIT_FILE")"
if [[ -z "$ROOT_TOKEN" || "$ROOT_TOKEN" == "null" ]]; then
  echo "❌ [bootstrap-vault] Root token not found in INIT_FILE"
  exit 1
fi
...
if ! vault_exec_with_token auth list -format=json 2>/dev/null | jq -e '.["kubernetes/"]' &>/dev/null; then
  ...
  echo "✅ [bootstrap-vault] Kubernetes auth already enabled."
fi
...
echo "✅ [bootstrap-vault] Kubernetes auth configured."
...
if ! vault_exec_with_token secrets list -format=json 2>/dev/null | jq -e ".[\"${KV_SECRETS_ENGINE_PATH}/\"]" &>/dev/null; then
  ...
  echo "✅ [bootstrap-vault] KV v2 enabled at $KV_SECRETS_ENGINE_PATH."
else
  echo "✅ [bootstrap-vault] KV v2 at $KV_SECRETS_ENGINE_PATH already enabled."
fi

echo "👉 [bootstrap-vault] Writing ACL policy"
