...
async function checkDiskSpace(): Promise<void> {
    ...
    logger.warn(
        {
            percentTrigger: PERCENT_TRIGGER_TO_REMOVE,
            percentToRemove,
            bytesToRemove,
        },
    ...
    try {
        ...
        const usedBytesAfterRemoval = (usageAfterRemoval.totalBlocks - usageAfterRemoval.availableBlocks) * usageAfterRemoval.blockSize;
        const occupiedPercentAfterRemoval = ((usageAfterRemoval.totalBlocks - usageAfterRemoval.availableBlocks) / usageAfterRemoval.totalBlocks) * 100;
