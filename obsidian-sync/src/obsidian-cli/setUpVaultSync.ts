...
const SYNCED_FILE_TYPES = ["image", "audio", "video", "pdf", "unsupported"].join(",");
...
export async function setUpVaultSync(): Promise<void> {
    ...
    logger.info(
        { vault: OBSIDIAN_VAULT_NAME, path: OBSIDIAN_VAULT_PATH },
        "🗂 Setting up vault",
    );
    ...
    const configFlags: string[][] = [
        ...
        ["--configs", SYNCED_CONFIGS],
        ["--file-types", SYNCED_FILE_TYPES],
    ];

    for (const flags of configFlags) {
        await runObsidianCommand(["sync-config", "--path", OBSIDIAN_VAULT_PATH, ...flags]);
    }
