...
export function registerShortLinkRoutes(server: VaultServer): void {
    server.post<{ Body: unknown }>("/short_links", async (request, reply) => {
        ...
        if (!isObject(body)) {
            return reply
                .code(400)
                .send({ status: "error", message: "Request body must be a JSON object" });
        }
        ...
        try {
            ...
            return reply
                .code(400)
                .send({ status: "error", message: 'Field "link" must be a valid URL' });
        }
        ...
        logger.info({ id, link: trimmedLink }, "✅ Short link added");
    ...
    server.get<{ Params: { id: string } }>("/short_links/:id", async (request, reply) => {
        ...
        if (!id) {
            return reply.code(400).send({ status: "error", message: "Missing short link id" });
        }
        ...
        if (url === null) {
            return reply.code(404).send({ status: "error", message: "Short link not found" });
        }

        return reply.code(200).send({ status: "ok", url });
    });
}
