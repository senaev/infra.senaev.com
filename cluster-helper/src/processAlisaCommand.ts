...
async function parseAlisaCommandWithOpenRouter(command: string): Promise<ParsedAlisaCommand> {
        ...
        messages: [
            {
                ...
                content: [
                    ...
                    ...TASKS_PROMPT_LINES,
                    '## Unclear commands',
                    'Most messages are short and have no verb at all. Do not give up on them: always pick the list that fits better.',
                    'A word or a list of words naming things — objects a person can buy, own or hold — is a shopping list. Example: \'молоко, хлеб, батарейки\' is type \'shopping\'.',
                    'A word or a list of words naming actions — something to be done, an errand, a call, a visit, a repair — is a task list. Example: \'позвонить врачу, вынести мусор\' is type \'task\'.',
                    'An amount, a unit or a brand next to a word is strong evidence of a thing to buy. A place, a person or a deadline next to a word is strong evidence of an action to do.',
                    'If one message holds both kinds, use the type that covers the larger part of it.',
                    '## Errors',
                    'Type \'error\' is the last resort, not the default. Use it only when the message names neither things nor actions — for example a greeting, a question, or unintelligible speech — and explain briefly in the error field.',
    ...
    if (!Array.isArray(parsed.items)) {
        throw new Error(`OpenRouter response items field is invalid [${JSON.stringify(parsed, null, 2)}]`);
    }

    if (!Array.isArray(parsed.tasks)) {
        throw new Error(`OpenRouter response tasks field is invalid [${JSON.stringify(parsed, null, 2)}]`);
    }
...
async function writeTasks({
    ...
    return {
        ...
        ...(await runWrite('add tasks', () =>
            addObsidianTasks(tasks.map(({ title, due_date }) => {
                return {
                    title: `${title} 🌱 ${source}`,
                    due_date,
                };
...
async function processShoppingOrTaskCommand(
    ...
    const openRouterError = parsed.type === 'error'
    ...
    logger.error({
        parsed,
        openRouterError,
    }, '❌ Cannot classify command, falling back');
...
async function processObsidianTaskCommand(
    ...
    logger.info({
        command,
        source,
    }, '👉 Start processing Alisa command (task-only)');
...
export async function processAlisaCommand(
    ...
    if (source === 'Obsidian Tasks') {
        return await processObsidianTaskCommand(command, source);
    }

    return await processShoppingOrTaskCommand(command, source);
}
