...
Services are pinned to specific VPS nodes via `nodeSelector` with label `vps=hetzner|yc|home`. Each service in `values.yaml` declares its target VPS.
...
Telegram webhook → `telegram-webhook-endpoint` (Node.js, port 3000) → Redpanda Kafka topic `telegram-webhook-data-topic` → `cluster-helper` consumer → actions (torrent downloads, notifications).
